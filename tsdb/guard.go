...
func (g *guard) Matches(points []model.Point) bool {
	if g == nil {
		return true
	}
...
// exprGuard is a union of cnosql.Expr based guards. a nil exprGuard matches
...
func newExprGuard(expr cnosql.Expr) *exprGuard {
	if expr == nil {
		return nil
	}
...
func newBinaryExprGuard(expr *cnosql.BinaryExpr) *exprGuard {
	// if it's a nested binary expression, always match.
	if _, ok := expr.LHS.(*cnosql.BinaryExpr); ok {
		return nil
	} else if _, ok := expr.RHS.(*cnosql.BinaryExpr); ok {
		return nil
	}

	// ensure one of the expressions is a VarRef, and make that the key.
	key, ok := expr.LHS.(*cnosql.VarRef)
	value := expr.RHS
	if !ok {
		key, ok = expr.RHS.(*cnosql.VarRef)
		if !ok {
			return nil
		}
		value = expr.LHS
	...
	if key.Val != "_name" && key.Type != cnosql.Unknown && key.Type != cnosql.Tag {
		return nil
	}
...
func (g *exprGuard) matches(pt model.Point) bool {
	switch {
	case g == nil:
		return true
