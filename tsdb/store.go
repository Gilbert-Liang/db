	...
	if ss.Cardinality() > 0 {
		...
		if sfile != nil {
			...
			if index.Type() == InmemIndexName {
				...
				var name []byte
				var tagsBuf model.Tags // Buffer for tags container.
				var err error
...
func (s *Store) DeleteMetric(database, name string) error {
	...
	return s.walkShards(shards, func(sh *Shard) error {
		...
		guard := newGuard(cnosql.MinTime, cnosql.MaxTime, []string{name}, nil)
...
func (s *Store) ExpandSources(sources cnosql.Sources) (cnosql.Sources, error) {
	shards := func() Shards {
		s.mu.RLock()
		defer s.mu.RUnlock()
...
func (s *Store) WriteToShard(shardID uint64, points []model.Point) error {
	s.mu.RLock()
...
func (s *Store) MetricNames(database string, cond cnosql.Expr) ([][]byte, error) {
	s.mu.RLock()
	shards := s.filterShards(byDatabase(database))
	s.mu.RUnlock()
	...
	is = is.DedupeInmemIndexes()
	return is.MetricNamesByExpr(cond)
}
...
func (s *Store) TagKeys(shardIDs []uint64, cond cnosql.Expr) ([]TagKeys, error) {
	if len(shardIDs) == 0 {
		return nil, nil
	}
	...
	filterExpr := cnosql.CloneExpr(cond)
	filterExpr = cnosql.Reduce(cnosql.RewriteExpr(filterExpr, func(e cnosql.Expr) cnosql.Expr {
		switch e := e.(type) {
		case *cnosql.BinaryExpr:
			switch e.Op {
			case cnosql.EQ, cnosql.NEQ, cnosql.EQREGEX, cnosql.NEQREGEX:
				tag, ok := e.LHS.(*cnosql.VarRef)
				if !ok || cnosql.IsSystemName(tag.Val) {
					return nil
				}
			}
		}
	...
	is = is.DedupeInmemIndexes()
	names, err := is.MetricNamesByExpr(metricExpr)
	if err != nil {
		return nil, err
	}
	...
	for _, name := range names {
		...
		values, err := is.MetricTagKeyValuesByExpr(name, keys, filterExpr, true)
		if err != nil {
			return nil, err
		}
...
func (s *Store) TagValues(shardIDs []uint64, cond cnosql.Expr) ([]TagValues, error) {
	if cond == nil {
	...
	for _, name := range names {
		...
		if result.values, err = is.MetricTagKeyValuesByExpr(name, result.keys, filterExpr, true); err != nil {
			return nil, err
		}
...
//
// TODO: a Tournament based merge (see: Knuth's TAOCP 5.4.1) might be more
	...
	for {
		...
		case <-t2.C:
			...
			s.walkShards(shards, func(sh *Shard) error {
				...
				names, err := indexSet.MetricNamesByExpr(nil)
				if err != nil {
