...
func (i *Index) MetricIterator() (tsdb.MetricIterator, error) {
	names, err := i.MetricNamesByExpr(nil)
	if err != nil {
		return nil, err
	}
...
func (i *Index) CreateSeriesListIfNotExists(seriesIDSet *tsdb.SeriesIDSet, metrics map[string]int,
	keys, names [][]byte, tagsSlice []model.Tags, opt *tsdb.EngineOptions, ignoreLimits bool) error {
...
// method.
func (i *Index) MetricTagKeyValuesByExpr(name []byte, keys []string, expr cnosql.Expr, keysSorted bool) ([][]string, error) {
	i.mu.RLock()
	mm := i.metrics[string(name)]
	i.mu.RUnlock()
...
func (i *Index) TagsForSeries(key string) (model.Tags, error) {
	i.mu.RLock()
	ss := i.series[key]
	i.mu.RUnlock()
...
func (i *Index) MetricNamesByExpr(expr cnosql.Expr) ([][]byte, error) {
	i.mu.RLock()
	defer i.mu.RUnlock()
	...
	return i.metricNamesByExpr(expr)
}
...
func (i *Index) metricNamesByExpr(expr cnosql.Expr) ([][]byte, error) {
	...
	case *cnosql.BinaryExpr:
		...
		case cnosql.EQ, cnosql.NEQ, cnosql.EQREGEX, cnosql.NEQREGEX:
			...
			if tag.Val == "_name" {
				return i.metricNamesByNameFilter(tf.Op, tf.Value, tf.Regex), nil
			} else if cnosql.IsSystemName(tag.Val) {
				return nil, nil
			}

			return i.metricNamesByTagFilters(tf), nil
		case cnosql.OR, cnosql.AND:
			lhs, err := i.metricNamesByExpr(e.LHS)
			if err != nil {
				return nil, err
			}

			rhs, err := i.metricNamesByExpr(e.RHS)
			if err != nil {
				return nil, err
			}
	...
	case *cnosql.ParenExpr:
		return i.metricNamesByExpr(e.Expr)
	}
	return nil, fmt.Errorf("%#v", expr)
}

// metricNamesByNameFilter returns the sorted metrics matching a name.
func (i *Index) metricNamesByNameFilter(op cnosql.Token, val string, regex *regexp.Regexp) [][]byte {
	var names [][]byte
	for _, m := range i.metrics {
		var matched bool
		switch op {
...
func (i *Index) metricNamesByTagFilters(filter *TagFilter) [][]byte {
	...
	for _, m := range i.metrics {
		...
		//     False  |       True      |      False
		//     False  |       False     |      True
		if tagMatch == (filter.Op == cnosql.EQ || filter.Op == cnosql.EQREGEX) {
			names = append(names, m.NameBytes)
		}
	}
...
func (i *Index) assignExistingSeries(shardID uint64, seriesIDSet *tsdb.SeriesIDSet, metrics map[string]int,
	keys, names [][]byte, tagsSlice []model.Tags) ([][]byte, [][]byte, []model.Tags) {
...
func (idx *ShardIndex) CreateSeriesListIfNotExists(keys, names [][]byte, tagsSlice []model.Tags) error {
...
func (idx *ShardIndex) InitializeSeries(keys, names [][]byte, tags []model.Tags) error {
...
// already present.
func (idx *ShardIndex) CreateSeriesIfNotExists(key, name []byte, tags model.Tags) error {
	return idx.Index.CreateSeriesListIfNotExists(idx.seriesIDSet, idx.metrics, [][]byte{key}, [][]byte{name}, []model.Tags{tags}, &idx.opt, false)
}
