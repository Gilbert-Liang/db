	...
	for len(lids) > 0 && len(rids) > 0 {
		...
		if lid == rid {
			...
			if lfilter != nil && rfilter != nil {
				be := &cnosql.BinaryExpr{
					Op:  cnosql.AND,
					LHS: lfilter,
					RHS: rfilter,
				}
				expr = cnosql.Reduce(be, nil)
			} else if lfilter != nil {
				expr = lfilter
			} else if rfilter != nil {
				expr = rfilter
...
func (m *metric) idsForExpr(n *cnosql.BinaryExpr) (seriesIDs, cnosql.Expr, error) {
	...
	name, ok := n.LHS.(*cnosql.VarRef)
	value := n.RHS
	if !ok {
		name, ok = n.RHS.(*cnosql.VarRef)
		if !ok {
	...
	if name.Val != "_name" && ((name.Type == cnosql.Unknown && m.HasField(name.Val)) || name.Type == cnosql.AnyField || (name.Type != cnosql.Tag && name.Type != cnosql.Unknown)) {
		...
		if value.Val != "_name" && ((value.Type == cnosql.Unknown && m.HasField(value.Val)) || name.Type == cnosql.AnyField || (value.Type != cnosql.Tag && value.Type != cnosql.Unknown)) {
			return m.SeriesIDs(), n, nil
		}
	}
	...
	if re, ok := value.(*cnosql.RegexLiteral); ok {
		...
		if name.Val == "_name" {
			...
			if (n.Op == cnosql.EQREGEX && match) || (n.Op == cnosql.NEQREGEX && !match) {
				return m.SeriesIDs(), &cnosql.BooleanLiteral{Val: true}, nil
			}
			return nil, nil, nil
		}
		...
		if empty && n.Op == cnosql.EQREGEX {
			...
			ids = sIDs.evict()
		} else if empty && n.Op == cnosql.NEQREGEX {
			...
			sort.Sort(ids)
		} else if !empty && n.Op == cnosql.EQREGEX {
			ids = make(seriesIDs, 0, len(m.SeriesIDs()))
			tagVals.RangeAll(func(k string, a seriesIDs) {
				if re.Val.MatchString(k) {
					ids = append(ids, a...)
	...
	// compare tag values
	if ref, ok := value.(*cnosql.VarRef); ok {
		var ids seriesIDs
...
type FilterExprs map[uint64]cnosql.Expr
...
func (fe FilterExprs) DeleteBoolLiteralTrues() {
	for id, expr := range fe {
		if e, ok := expr.(*cnosql.BooleanLiteral); ok && e.Val {
			delete(fe, id)
		}
	}
}
...
func (m *metric) WalkWhereForSeriesIds(expr cnosql.Expr) (seriesIDs, FilterExprs, error) {
	...
	case *cnosql.BinaryExpr:
		switch n.Op {
		case cnosql.EQ, cnosql.NEQ, cnosql.LT, cnosql.LTE, cnosql.GT, cnosql.GTE, cnosql.EQREGEX, cnosql.NEQREGEX:
			...
			if b, ok := expr.(*cnosql.BooleanLiteral); ok && b.Val {
				expr = nil
			}
...
func newSeries(id uint64, m *metric, key string, tags model.Tags) *series {
	return &series{
		ID:     id,
		Metric: m,
		Key:    key,
...
func (m *metric) TagValues(key string) []string {
	m.mu.RLock()
	defer m.mu.RUnlock()
