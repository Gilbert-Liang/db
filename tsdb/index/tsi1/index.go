...
func (i *Index) CreateSeriesListIfNotExists(keys [][]byte, names [][]byte, tagsSlice []model.Tags) error {
	...
	pTags := make([][]model.Tags, i.PartitionN)
...
func (i *Index) CreateSeriesIfNotExists(key, name []byte, tags model.Tags) error {
	ids, err := i.partition(key).createSeriesListIfNotExists([][]byte{name}, []model.Tags{tags})
	if err != nil {
		return err
	}
...
func (i *Index) InitializeSeries(keys, names [][]byte, tags []model.Tags) error {
	return nil
}
...
func (i *Index) DropSeries(seriesID uint64, key []byte, cascade bool) error {
	...
	name, tags := model.ParseKeyBytes(key)
