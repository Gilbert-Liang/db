...
// bulk.
func (p *Partition) createSeriesListIfNotExists(names [][]byte, tagsSlice []model.Tags) ([]uint64, error) {
...
func (p *Partition) TagKeyIterator(name []byte) tsdb.TagKeyIterator {
	fs, err := p.RetainFileSet()
	if err != nil {
		return nil // TODO: this should probably return an error.
	}
...
// compact compacts contiguous groups of files that are not currently compacting.
func (p *Partition) compact() {
	if p.isClosing() {
		return
