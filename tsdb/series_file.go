...
func (f *SeriesFile) CreateSeriesListIfNotExists(names [][]byte, tagsSlice []model.Tags) ([]uint64, error) {
...
func (f *SeriesFile) Series(id uint64) ([]byte, model.Tags) {
	key := f.SeriesKey(id)
	if key == nil {
		return nil, nil
	}
...
func (f *SeriesFile) SeriesID(name []byte, tags model.Tags, buf []byte) uint64 {
...
func (f *SeriesFile) HasSeries(name []byte, tags model.Tags, buf []byte) bool {
...
func AppendSeriesKey(dst []byte, name []byte, tags model.Tags) []byte {
...
func ParseSeriesKey(data []byte) (name []byte, tags model.Tags) {
...
func ParseSeriesKeyInto(data []byte, dstTags model.Tags) ([]byte, model.Tags) {
...
func parseSeriesKey(data []byte, dst model.Tags) ([]byte, model.Tags) {
	var name []byte
	...
	if got, want := len(dst), tagN; got < want {
		dst = append(dst, make(model.Tags, want-got)...)
	} else if got > want {
		dst = dst[:want]
	}
	dst = dst[:tagN]
...
func GenerateSeriesKeys(names [][]byte, tagsSlice []model.Tags) [][]byte {
...
func SeriesKeysSize(names [][]byte, tagsSlice []model.Tags) int {
	var n int
	for i := range names {
...
func SeriesKeySize(name []byte, tags model.Tags) int {
	var n int
	n += 2 + len(name)
