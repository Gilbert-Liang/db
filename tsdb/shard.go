...
func (s *Shard) WritePoints(points []model.Point) error {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Shard) validateSeriesAndFields(points []model.Point) ([]model.Point, []*FieldCreate, error) {
	var (
	...
	tagsSlice := make([]model.Tags, len(points))
	...
	for i, p := range points {
		...
		if validateKeys && !model.ValidKeyTokens(string(p.Name()), tags) {
			dropped++
			if reason == "" {
	...
	if err := engine.CreateSeriesListIfNotExists(keys, names, tagsSlice); err != nil {
		switch err := err.(type) {
		// TODO: why is this a *PartialWriteError when everything else is not a pointer?
...
func (s *Shard) DeleteSeriesRangeWithPredicate(itr SeriesIterator, predicate func(name []byte, tags model.Tags) (int64, int64, bool)) error {
	engine, err := s.Engine()
	if err != nil {
		return err
	}
...
// provided expression.
func (s *Shard) MetricTagKeyValuesByExpr(name []byte, key []string, expr cnosql.Expr, keysSorted bool) ([][]string, error) {
	index, err := s.Index()
	if err != nil {
		return nil, err
	}
	indexSet := IndexSet{Indexes: []Index{index}, SeriesFile: s.sfile}
	return indexSet.MetricTagKeyValuesByExpr(name, key, expr, keysSorted)
}
...
func (s *Shard) CreateIterator(ctx context.Context, m *cnosql.Metric, opt query.IteratorOptions) (query.Iterator, error) {
	...
	case "_series":
		// TODO: Move up to the Shards.CreateIterator().
		index, err := s.Index()
		if err != nil {
			return nil, err
		}
...
func (s *Shard) CreateSeriesCursor(ctx context.Context, req SeriesCursorRequest, cond cnosql.Expr) (SeriesCursor, error) {
	index, err := s.Index()
	if err != nil {
		return nil, err
	}
...
func (s *Shard) FieldDimensions(metrics []string) (fields map[string]cnosql.DataType, dimensions map[string]struct{}, err error) {
	...
	for _, name := range metrics {
		...
		if strings.HasPrefix(name, "_") {
			...
			if len(keys) > 0 {
				for _, k := range keys {
					if fields[k].LessThan(cnosql.String) {
						fields[k] = cnosql.String
					}
				}
				continue
			}
...
func (s *Shard) expandSources(sources cnosql.Sources) (cnosql.Sources, error) {
	...
	expanded := make(cnosql.Sources, 0, len(set))
	for _, name := range names {
...
func (a Shards) CallType(name string, args []cnosql.DataType) (cnosql.DataType, error) {
...
func (a Shards) CreateSeriesCursor(ctx context.Context, req SeriesCursorRequest, cond cnosql.Expr) (_ SeriesCursor, err error) {
	var (
		idxs  []Index
		sfile *SeriesFile
	)
...
func (a Shards) ExpandSources(sources cnosql.Sources) (cnosql.Sources, error) {
	// Use a map as a set to prevent duplicates.
	set := map[string]cnosql.Source{}
	...
	// Convert set to a list of Sources.
	sorted := make([]cnosql.Source, 0, len(set))
	for _, name := range names {
...
func (fs *MetricFieldSet) saveNoLock() error {
	...
	for name, mf := range fs.fields {
		...
		mf.ForEachField(func(field string, typ cnosql.DataType) bool {
...
func NewFieldKeysIterator(sh *Shard, opt query.IteratorOptions) (query.Iterator, error) {
	...
	names, err := indexSet.MetricNamesByExpr(opt.Condition)
	if err != nil {
		return nil, err
	}
	itr.names = names
