...
// CnosQLType returns the cnosql.DataType the values map to.
func (a Values) CnosQLType() (cnosql.DataType, error) {
	if len(a) == 0 {
		return cnosql.Unknown, fmt.Errorf("no values to infer type")
	}
	...
	case UnsignedValue:
		return cnosql.Unsigned, nil
	case BooleanValue:
		return cnosql.Boolean, nil
	case StringValue:
		return cnosql.String, nil
	}

	return cnosql.Unknown, fmt.Errorf("unsupported value type %T", a[0])
}
