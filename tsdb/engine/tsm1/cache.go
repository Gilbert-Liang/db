...
// largest box we could imagine running cnosdb-cli.
const ringShards = 16
...
// CnosQLType returns for the entry the data type of its values.
func (e *entry) CnosQLType() (cnosql.DataType, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.values.CnosQLType()
}
...
type Cache struct {
	...
	// This number is the number of pending or failed WriteSnapshot attempts since the last successful one.
...
func (c *Cache) Type(key []byte) (model.FieldType, error) {
	c.mu.RLock()
	e := c.store.entry(key)
	...
	if e != nil {
		typ, err := e.CnosQLType()
		if err != nil {
			return model.Empty, tsdb.ErrUnknownFieldType
		}
...
//
// TODO: Lock usage could possibly be optimised if necessary.
