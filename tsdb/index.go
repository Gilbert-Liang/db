...
type Index interface {
	...
	InitializeSeries(keys, names [][]byte, tags []model.Tags) error
	CreateSeriesIfNotExists(key, name []byte, tags model.Tags) error
	CreateSeriesListIfNotExists(keys, names [][]byte, tags []model.Tags) error
...
type SeriesElem interface {
	...
	// CnosQL expression associated with series during filtering.
	Expr() cnosql.Expr
}
...
func (e *seriesElemAdapter) Tags() model.Tags  { return e.tags }
...
// seriesQueryAdapterIterator adapts SeriesIDIterator to an cnosql.Iterator.
...
func (itr *seriesQueryAdapterIterator) Next() (*query.FloatPoint, error) {
	for {
		...
		key := string(model.MakeKey(name, tags))
...
func newSeriesIDExprIterator(itr SeriesIDIterator, expr cnosql.Expr) SeriesIDIterator {
	if itr == nil {
		return nil
	}
...
func (itr *seriesIDIntersectIterator) Next() (_ SeriesIDElem, err error) {
	for {
		...
		if expr0 == nil {
			...
			elem.Expr = cnosql.Reduce(&cnosql.BinaryExpr{
				Op:  cnosql.AND,
				LHS: expr0,
				RHS: expr1,
			}, nil)
...
// seriesPointIterator adapts SeriesIterator to an cnosql.Iterator.
...
func (is IndexSet) MetricNamesByExpr(expr cnosql.Expr) ([][]byte, error) {
	release := is.SeriesFile.Retain()
	defer release()

	// Return filtered list if expression exists.
	if expr != nil {
		names, err := is.metricNamesByExpr(expr)
		if err != nil {
			return nil, err
		}
...
func (is IndexSet) metricNamesByExpr(expr cnosql.Expr) ([][]byte, error) {
	...
	case *cnosql.BinaryExpr:
		...
		case cnosql.EQ, cnosql.NEQ, cnosql.EQREGEX, cnosql.NEQREGEX:
			...
			if tag.Val == "_name" {
				return is.metricNamesByNameFilter(e.Op, value, regex)
			} else if cnosql.IsSystemName(tag.Val) {
				return nil, nil
			}
			return is.metricNamesByTagFilter(e.Op, tag.Val, value, regex)

		case cnosql.OR, cnosql.AND:
			lhs, err := is.metricNamesByExpr(e.LHS)
			if err != nil {
				return nil, err
			}

			rhs, err := is.metricNamesByExpr(e.RHS)
			if err != nil {
				return nil, err
			}
	...
	case *cnosql.ParenExpr:
		return is.metricNamesByExpr(e.Expr)
	default:
...
func (is IndexSet) metricNamesByNameFilter(op cnosql.Token, val string, regex *regexp.Regexp) ([][]byte, error) {
	itr, err := is.metricIterator()
	if err != nil {
		return nil, err
	} else if itr == nil {
...
func (is IndexSet) metricNamesByTagFilter(op cnosql.Token, key, val string, regex *regexp.Regexp) ([][]byte, error) {
	var names [][]byte
	...
	for {
		...
		//     False  |       True      |      False
		//     False  |       False     |      True
		if tagMatch == (op == cnosql.EQ || op == cnosql.EQREGEX) {
...
func (is IndexSet) MetricSeriesKeysByExpr(name []byte, expr cnosql.Expr) ([][]byte, error) {
	...
	for {
		...
		if e.Expr != nil {
			if v, ok := e.Expr.(*cnosql.BooleanLiteral); !ok || !v.Val {
		...
		name, tags := ParseSeriesKey(seriesKey)
		keys = append(keys, model.MakeKey(name, tags))
	}
...
func (is IndexSet) seriesByExprIterator(name []byte, expr cnosql.Expr) (SeriesIDIterator, error) {
...
func (is IndexSet) seriesByBinaryExprIterator(name []byte, n *cnosql.BinaryExpr) (SeriesIDIterator, error) {
	...
	key, ok := n.LHS.(*cnosql.VarRef)
	value := n.RHS
	if !ok {
		key, ok = n.RHS.(*cnosql.VarRef)
		if !ok {
...
func (is IndexSet) seriesByBinaryExprRegexIterator(name, key []byte, value *regexp.Regexp, op cnosql.Token) (SeriesIDIterator, error) {
	...
	if bytes.Equal(key, []byte("_name")) {
		...
		if (op == cnosql.EQREGEX && match) || (op == cnosql.NEQREGEX && !match) {
			...
			return newSeriesIDExprIterator(mitr, &cnosql.BooleanLiteral{Val: true}), nil
		}
		return nil, nil
	}
	return is.matchTagValueSeriesIDIterator(name, key, value, op == cnosql.EQREGEX)
}

func (is IndexSet) seriesByBinaryExprVarRefIterator(name, key []byte, value *cnosql.VarRef, op cnosql.Token) (SeriesIDIterator, error) {
...
// lexicographic order.
func (is IndexSet) TagValuesByKeyAndExpr(name []byte, keys []string, expr cnosql.Expr, fieldset *MetricFieldSet) ([]map[string]struct{}, error) {
	release := is.SeriesFile.Retain()
	defer release()
	return is.tagValuesByKeyAndExpr(name, keys, expr)
}
...
// series file.
func (is IndexSet) tagValuesByKeyAndExpr(name []byte, keys []string, expr cnosql.Expr) ([]map[string]struct{}, error) {
	valueExpr := cnosql.CloneExpr(expr)
	valueExpr = cnosql.Reduce(cnosql.RewriteExpr(valueExpr, func(e cnosql.Expr) cnosql.Expr {
		switch e := e.(type) {
		case *cnosql.BinaryExpr:
			switch e.Op {
			case cnosql.EQ, cnosql.NEQ, cnosql.EQREGEX, cnosql.NEQREGEX:
	...
	for {
		...
		for i := 0; i < tagN; i++ {
			...
			if valueExpr != nil {
				if !cnosql.EvalBool(valueExpr, map[string]interface{}{"value": string(value)}) {
					continue
				}
			}
...
func (is IndexSet) MetricTagKeyValuesByExpr(name []byte, keys []string, expr cnosql.Expr, keysSorted bool) ([][]string, error) {
	if len(keys) == 0 {
		return nil, nil
	}
	...
	resultSet, err := is.tagValuesByKeyAndExpr(name, keys, expr)
	if err != nil {
		return nil, err
	}
...
func (is IndexSet) TagSets(sfile *SeriesFile, name []byte, opt query.IteratorOptions) ([]*query.TagSet, error) {
	...
	var keyBuf []byte
	var tagsBuf model.Tags // Buffer for tags. Tags are not needed outside of each loop iteration.
	for {
		se, err := itr.Next()
		if err != nil {
			return nil, err
		...
		keyBuf = model.AppendMakeKey(keyBuf, name, tagsBuf)
