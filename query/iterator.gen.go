...
func newFloatIteratorScanner(input FloatIterator, keys []cnosql.VarRef, defaultValue interface{}) *floatIteratorScanner {
...
func newFloatFillIterator(input FloatIterator, expr cnosql.Expr, opt IteratorOptions) *floatFillIterator {
	if opt.Fill == cnosql.NullFill {
		if expr, ok := expr.(*cnosql.Call); ok && expr.Name == "count" {
			opt.Fill = cnosql.NumberFill
...
func (itr *floatFillIterator) Next() (*FloatPoint, error) {
	if !itr.init {
		...
		if itr.startTime == cnosql.MinTime {
	...
	if p == nil || p.Name != itr.window.name || p.Tags.ID() != itr.window.tags.ID() {
		...
		if itr.opt.Ascending && itr.window.time <= itr.endTime {
			...
			p = nil
			goto CONSTRUCT
		} else if !itr.opt.Ascending && itr.window.time >= itr.endTime && itr.endTime != cnosql.MinTime {
			itr.input.unread(p)
			p = nil
			goto CONSTRUCT
		}
		...
		itr.window.name, itr.window.tags = p.Name, p.Tags
		itr.window.time = itr.startTime
		if itr.window.time == cnosql.MinTime {
			itr.window.time, _ = itr.opt.Window(p.Time)
		}
		if itr.opt.Location != nil {
			_, itr.window.offset = itr.opt.Zone(itr.window.time)
...
func newFloatFilterIterator(input FloatIterator, cond cnosql.Expr, opt IteratorOptions) FloatIterator {
	// Strip out time conditions from the WHERE clause.
	// TODO: This should really be done for us when creating the IteratorOptions struct.
	n := cnosql.RewriteFunc(cnosql.CloneExpr(cond), func(n cnosql.Node) cnosql.Node {
		switch n := n.(type) {
	...
	cond, _ = n.(cnosql.Expr)
	if cond == nil {
		return input
	} else if n, ok := cond.(*cnosql.BooleanLiteral); ok && n.Val {
		return input
	}
...
func (itr *floatFilterIterator) Next() (*FloatPoint, error) {
	for {
		...
		if !cnosql.EvalBool(itr.cond, itr.m) {
			continue
		}
		return p, nil
	}
...
func newIntegerIteratorScanner(input IntegerIterator, keys []cnosql.VarRef, defaultValue interface{}) *integerIteratorScanner {
...
func newIntegerFillIterator(input IntegerIterator, expr cnosql.Expr, opt IteratorOptions) *integerFillIterator {
	if opt.Fill == cnosql.NullFill {
		if expr, ok := expr.(*cnosql.Call); ok && expr.Name == "count" {
			opt.Fill = cnosql.NumberFill
...
func newIntegerFilterIterator(input IntegerIterator, cond cnosql.Expr, opt IteratorOptions) IntegerIterator {
	// Strip out time conditions from the WHERE clause.
	// TODO: This should really be done for us when creating the IteratorOptions struct.
	n := cnosql.RewriteFunc(cnosql.CloneExpr(cond), func(n cnosql.Node) cnosql.Node {
		switch n := n.(type) {
...
func newUnsignedIteratorScanner(input UnsignedIterator, keys []cnosql.VarRef, defaultValue interface{}) *unsignedIteratorScanner {
...
func newUnsignedFillIterator(input UnsignedIterator, expr cnosql.Expr, opt IteratorOptions) *unsignedFillIterator {
	if opt.Fill == cnosql.NullFill {
		if expr, ok := expr.(*cnosql.Call); ok && expr.Name == "count" {
			opt.Fill = cnosql.NumberFill
...
func newUnsignedFilterIterator(input UnsignedIterator, cond cnosql.Expr, opt IteratorOptions) UnsignedIterator {
	// Strip out time conditions from the WHERE clause.
	// TODO: This should really be done for us when creating the IteratorOptions struct.
	n := cnosql.RewriteFunc(cnosql.CloneExpr(cond), func(n cnosql.Node) cnosql.Node {
		switch n := n.(type) {
...
func newStringIteratorScanner(input StringIterator, keys []cnosql.VarRef, defaultValue interface{}) *stringIteratorScanner {
...
func newStringFillIterator(input StringIterator, expr cnosql.Expr, opt IteratorOptions) *stringFillIterator {
	if opt.Fill == cnosql.NullFill {
		if expr, ok := expr.(*cnosql.Call); ok && expr.Name == "count" {
			opt.Fill = cnosql.NumberFill
			opt.FillValue = ""
...
func newStringFilterIterator(input StringIterator, cond cnosql.Expr, opt IteratorOptions) StringIterator {
	// Strip out time conditions from the WHERE clause.
	// TODO: This should really be done for us when creating the IteratorOptions struct.
	n := cnosql.RewriteFunc(cnosql.CloneExpr(cond), func(n cnosql.Node) cnosql.Node {
		switch n := n.(type) {
...
func newBooleanIteratorScanner(input BooleanIterator, keys []cnosql.VarRef, defaultValue interface{}) *booleanIteratorScanner {
...
func newBooleanFillIterator(input BooleanIterator, expr cnosql.Expr, opt IteratorOptions) *booleanFillIterator {
	if opt.Fill == cnosql.NullFill {
		if expr, ok := expr.(*cnosql.Call); ok && expr.Name == "count" {
			opt.Fill = cnosql.NumberFill
			opt.FillValue = false
...
func newBooleanFilterIterator(input BooleanIterator, cond cnosql.Expr, opt IteratorOptions) BooleanIterator {
	// Strip out time conditions from the WHERE clause.
	// TODO: This should really be done for us when creating the IteratorOptions struct.
	n := cnosql.RewriteFunc(cnosql.CloneExpr(cond), func(n cnosql.Node) cnosql.Node {
		switch n := n.(type) {
