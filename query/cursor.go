...
func RowCursor(rows []Row, columns []cnosql.VarRef) Cursor {
	return &rowCursor{
		rows:    rows,
		columns: columns,
	}
...
func (cur *rowCursor) Columns() []cnosql.VarRef {
	return cur.columns
}
...
func newScannerCursorBase(scan scannerFunc, fields []*cnosql.Field, loc *time.Location) scannerCursorBase {
	typmap := FunctionTypeMapper{}
	exprs := make([]cnosql.Expr, len(fields))
	columns := make([]cnosql.VarRef, len(fields))
	for i, f := range fields {
		exprs[i] = f.Expr
		columns[i] = cnosql.VarRef{
			Val:  f.Name(),
			Type: cnosql.EvalType(f.Expr, nil, typmap),
		}
	}
	if loc == nil {
		loc = time.UTC
...
func (cur *scannerCursorBase) Columns() []cnosql.VarRef {
	return cur.columns
}
...
func newScannerCursor(s IteratorScanner, fields []*cnosql.Field, opt IteratorOptions) *scannerCursor {
...
func newMultiScannerCursor(scanners []IteratorScanner, fields []*cnosql.Field, opt IteratorOptions) *multiScannerCursor {
...
func newFilterCursor(cur Cursor, filter cnosql.Expr) *filterCursor {
	fields := make(map[string]IteratorMap)
	for _, name := range cnosql.ExprNames(filter) {
...
func newNullCursor(fields []*cnosql.Field) *nullCursor {
	columns := make([]cnosql.VarRef, len(fields))
	for i, f := range fields {
...
func (cur *nullCursor) Columns() []cnosql.VarRef {
	return cur.columns
}
