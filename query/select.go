...
func Prepare(stmt *cnosql.SelectStatement, shardMapper ShardMapper, opt SelectOptions) (PreparedStatement, error) {
...
func Select(ctx context.Context, stmt *cnosql.SelectStatement, shardMapper ShardMapper, opt SelectOptions) (Cursor, error) {
...
func (p *preparedStatement) Select(ctx context.Context) (Cursor, error) {
	// TODO: Remove this hacky method of propagating now.
	...
	if m := MonitorFromContext(ctx); m != nil {
		if p.maxPointN > 0 {
			monitor := PointLimitMonitor(cur, time.Second, p.maxPointN)
			m.Monitor(monitor)
		}
	}
	return cur, nil
...
func buildExprIterator(ctx context.Context, expr cnosql.Expr, ic IteratorCreator, sources cnosql.Sources, opt IteratorOptions, selector, writeMode bool) (Iterator, error) {
	opt.Expr = expr
...
func (b *exprIteratorBuilder) buildVarRefIterator(ctx context.Context, expr *cnosql.VarRef) (Iterator, error) {
...
func (b *exprIteratorBuilder) buildCallIterator(ctx context.Context, expr *cnosql.Call) (Iterator, error) {
	// TODO: Refactor this. This section needs to die in a fire.
	opt := b.opt
	...
	case "distinct":
		opt.Ordered = true
		input, err := buildExprIterator(ctx, expr.Args[0].(*cnosql.VarRef), b.ic, b.sources, opt, b.selector, false)
		if err != nil {
			return nil, err
		}
	...
	case "sample":
		...
		size := expr.Args[1].(*cnosql.IntegerLiteral)
	...
	case "derivative", "non_negative_derivative", "difference", "non_negative_difference", "moving_average", "exponential_moving_average", "double_exponential_moving_average", "triple_exponential_moving_average", "relative_strength_index", "triple_exponential_derivative", "kaufmans_efficiency_ratio", "kaufmans_adaptive_moving_average", "chande_momentum_oscillator", "elapsed":
		...
		case "moving_average":
			n := expr.Args[1].(*cnosql.IntegerLiteral)
	...
	case "integral":
		opt.Ordered = true
		input, err := buildExprIterator(ctx, expr.Args[0].(*cnosql.VarRef), b.ic, b.sources, opt, false, false)
		if err != nil {
			return nil, err
		}
	...
	case "top":
		...
		n := expr.Args[len(expr.Args)-1].(*cnosql.IntegerLiteral)
	...
	if !b.selector || !opt.Interval.IsZero() {
		...
		if !opt.Interval.IsZero() && opt.Fill != cnosql.NoFill {
...
func (b *exprIteratorBuilder) callIterator(ctx context.Context, expr *cnosql.Call, opt IteratorOptions) (Iterator, error) {
	inputs := make([]Iterator, 0, len(b.sources))
	if err := func() error {
		for _, source := range b.sources {
			switch source := source.(type) {
			...
			case *cnosql.SubQuery:
				...
				arg0 := expr.Args[0].(*cnosql.VarRef)

				opt.Ordered = false
				input, err := buildExprIterator(ctx, arg0, b.ic, []cnosql.Source{source}, opt, b.selector, false)
				if err != nil {
					return err
				}
...
func buildCursor(ctx context.Context, stmt *cnosql.SelectStatement, ic IteratorCreator, opt IteratorOptions) (Cursor, error) {
	...
	for _, f := range stmt.Fields {
		...
		case *cnosql.Call:
			if expr.Name == "top" || expr.Name == "bottom" {
				for i := 1; i < len(expr.Args)-1; i++ {
					nf := cnosql.Field{Expr: expr.Args[i]}
	...
	var auxKeys []cnosql.VarRef
	if len(valueMapper.refs) > 0 {
		opt.Aux = make([]cnosql.VarRef, 0, len(valueMapper.refs))
		for ref := range valueMapper.refs {
			opt.Aux = append(opt.Aux, *ref)
		}
		sort.Sort(cnosql.VarRefs(opt.Aux))

		auxKeys = make([]cnosql.VarRef, len(opt.Aux))
	...
	for call := range valueMapper.calls {
		...
		keys := make([]cnosql.VarRef, 0, len(auxKeys)+1)
...
func buildAuxIterator(ctx context.Context, ic IteratorCreator, sources cnosql.Sources, opt IteratorOptions) (Iterator, error) {
	span := tracing.SpanFromContext(ctx)
	if span != nil {
...
func buildFieldIterator(ctx context.Context, expr cnosql.Expr, ic IteratorCreator, sources cnosql.Sources, opt IteratorOptions, selector, writeMode bool) (Iterator, error) {
	span := tracing.SpanFromContext(ctx)
	if span != nil {
		span = span.StartSpan("iterator_scanner")
		defer span.Finish()
...
type valueMapper struct {
	...
	symbols map[string]cnosql.VarRef
	// An index that maps a specific expression to a symbol. This ensures that
	// only expressions that were mapped get symbolized.
	table map[cnosql.Expr]cnosql.VarRef
	// A collection of all of the calls in the table.
	calls map[*cnosql.Call]struct{}
	// A collection of all of the calls in the table.
	refs map[*cnosql.VarRef]struct{}
	i    int
}

func newValueMapper() *valueMapper {
	return &valueMapper{
		symbols: make(map[string]cnosql.VarRef),
		table:   make(map[cnosql.Expr]cnosql.VarRef),
		calls:   make(map[*cnosql.Call]struct{}),
		refs:    make(map[*cnosql.VarRef]struct{}),
	}
}

func (v *valueMapper) Map(field *cnosql.Field) *cnosql.Field {
	clone := *field
	clone.Expr = cnosql.CloneExpr(field.Expr)

	cnosql.Walk(v, clone.Expr)
	clone.Expr = cnosql.RewriteExpr(clone.Expr, v.rewriteExpr)
	return &clone
}

func (v *valueMapper) Visit(n cnosql.Node) cnosql.Visitor {
	expr, ok := n.(cnosql.Expr)
	if !ok {
		return v
	}
...
func (v *valueMapper) rewriteExpr(expr cnosql.Expr) cnosql.Expr {
...
func validateTypes(stmt *cnosql.SelectStatement) error {
	valuer := cnosql.TypeValuerEval{
		TypeMapper: cnosql.MultiTypeMapper(
			FunctionTypeMapper{},
			MathTypeMapper{},
		),
	}
...
// in the slice.
func hasValidType(refs []cnosql.VarRef) bool {
	for _, ref := range refs {
		if ref.Type != cnosql.Unknown {
			return true
		}
	}
	return false
