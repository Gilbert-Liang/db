...
func (a Iterators) dataType() cnosql.DataType {
	if len(a) == 0 {
...
func NewFilterIterator(input Iterator, cond cnosql.Expr, opt IteratorOptions) Iterator {
	if input == nil {
		return nil
	}
...
func NewFillIterator(input Iterator, expr cnosql.Expr, opt IteratorOptions) Iterator {
	switch input := input.(type) {
	case FloatIterator:
...
func NewIteratorScanner(input Iterator, keys []cnosql.VarRef, defaultValue interface{}) IteratorScanner {
	switch input := input.(type) {
	case FloatIterator:
...
type IteratorOptions struct {
	...
	// Fill options.
	Fill      cnosql.FillOption
	FillValue interface{}

	// Condition to filter by.
	Condition cnosql.Expr
...
func newIteratorOptionsStmt(stmt *cnosql.SelectStatement, sopt SelectOptions) (opt IteratorOptions, err error) {
	// Determine time range from the condition.
	valuer := &cnosql.NowValuer{Location: stmt.Location}
	condition, timeRange, err := cnosql.ConditionExpr(stmt.Condition, valuer)
	if err != nil {
	...
	for _, d := range stmt.Dimensions {
		if d, ok := d.Expr.(*cnosql.VarRef); ok {
	...
	if opt.Fill == cnosql.NullFill && stmt.Target != nil {
...
func newIteratorOptionsSubstatement(ctx context.Context, stmt *cnosql.SelectStatement, opt IteratorOptions) (IteratorOptions, error) {
	...
	if !subOpt.Interval.IsZero() && subOpt.EndTime == cnosql.MaxTime {
	...
	valuer := &cnosql.NowValuer{Location: stmt.Location}
	cond, t, err := cnosql.ConditionExpr(stmt.Condition, valuer)
	if err != nil {
		return IteratorOptions{}, err
	}
	...
	// raw queries.
	if !stmt.IsRawQuery && subOpt.Fill == cnosql.NullFill {
		subOpt.Fill = cnosql.NoFill
	}
...
func (opt IteratorOptions) Window(t int64) (start, end int64) {
	...
	// Find the start time.
	if cnosql.MinTime+dt >= t {
		start = cnosql.MinTime
	} else {
		start = t - dt
	}
	...
	// Find the end time.
	if dt := int64(opt.Interval.Duration) - dt; cnosql.MaxTime-dt <= t {
		end = cnosql.MaxTime
	} else {
		end = t + dt
	}
...
func (opt IteratorOptions) DerivativeInterval() Interval {
	...
	if expr, ok := opt.Expr.(*cnosql.Call); ok && len(expr.Args) == 2 {
		return Interval{Duration: expr.Args[1].(*cnosql.DurationLiteral).Val}
	}
...
func decodeIteratorOptions(pb *internal.IteratorOptions) (*IteratorOptions, error) {
	opt := &IteratorOptions{
		...
		Fill:       cnosql.FillOption(pb.GetFill()),
	...
	// Set expression, if set.
	if pb.Expr != nil {
		expr, err := cnosql.ParseExpr(pb.GetExpr())
		if err != nil {
			return nil, err
		}
		opt.Expr = expr
	...
	if fields := pb.GetFields(); fields != nil {
		opt.Aux = make([]cnosql.VarRef, len(fields))
		for i, ref := range fields {
			opt.Aux[i] = decodeVarRef(ref)
		}
	} else if aux := pb.GetAux(); aux != nil {
		opt.Aux = make([]cnosql.VarRef, len(aux))
		for i, name := range aux {
			opt.Aux[i] = cnosql.VarRef{Val: name}
		}
	}
	...
	// Set condition, if set.
	if pb.Condition != nil {
		expr, err := cnosql.ParseExpr(pb.GetCondition())
		if err != nil {
			return nil, err
		}
		opt.Condition = expr
...
func encodeVarRef(ref cnosql.VarRef) *internal.VarRef {
...
func decodeVarRef(pb *internal.VarRef) cnosql.VarRef {
	return cnosql.VarRef{
		Val:  pb.GetVal(),
		Type: cnosql.DataType(pb.GetType()),
	}
}
