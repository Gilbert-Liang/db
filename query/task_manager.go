...
func (t *TaskManager) ExecuteStatement(ctx *ExecutionContext, stmt cnosql.Statement) error {
	switch stmt := stmt.(type) {
	case *cnosql.ShowQueriesStatement:
	...
	case *cnosql.KillQueryStatement:
...
func (t *TaskManager) executeKillQueryStatement(stmt *cnosql.KillQueryStatement) error {
	return t.KillQuery(stmt.QueryID)
}

func (t *TaskManager) executeShowQueriesStatement(q *cnosql.ShowQueriesStatement) (model.Rows, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *TaskManager) AttachQuery(q *cnosql.Query, opt ExecutionOptions, interrupt <-chan struct{}) (*ExecutionContext, func(), error) {
	t.mu.Lock()
	defer t.mu.Unlock()
