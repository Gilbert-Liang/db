...
func (b *subqueryBuilder) mapAuxFields(auxFields []cnosql.VarRef) []IteratorMap {
...
func (b *subqueryBuilder) mapAuxField(name *cnosql.VarRef) IteratorMap {
	offset := 0
	for i, f := range b.stmt.Fields {
		...
		} else if call, ok := f.Expr.(*cnosql.Call); ok && (call.Name == "top" || call.Name == "bottom") {
			// We may match one of the arguments in "top" or "bottom".
			if len(call.Args) > 2 {
				for j, arg := range call.Args[1 : len(call.Args)-1] {
					if arg, ok := arg.(*cnosql.VarRef); ok && arg.Val == name.Val {
						return FieldMap{
							Index: i + j + 1,
							Type:  cnosql.String,
						}
	...
	for _, d := range b.stmt.Dimensions {
		if d, ok := d.Expr.(*cnosql.VarRef); ok && name.Val == d.Val {
			return TagMap(d.Val)
		}
	}
...
func (b *subqueryBuilder) buildVarRefIterator(ctx context.Context, expr *cnosql.VarRef, opt IteratorOptions) (Iterator, error) {
