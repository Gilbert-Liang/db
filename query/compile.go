...
func Compile(stmt *cnosql.SelectStatement, opt CompileOptions) (Statement, error) {
	c := newCompiler(opt)
	c.stmt = stmt.Clone()
...
func (c *compiledStatement) preprocess(stmt *cnosql.SelectStatement) error {
	c.Ascending = stmt.TimeAscending()
	c.Limit = stmt.Limit
	c.HasTarget = stmt.Target != nil

	valuer := cnosql.NowValuer{Now: c.Options.Now, Location: stmt.Location}
	...
	if c.TimeRange.Min.IsZero() {
		c.TimeRange.Min = time.Unix(0, cnosql.MinTime).UTC()
	}
	if c.TimeRange.Max.IsZero() {
		...
		if !c.Interval.IsZero() {
			...
			c.TimeRange.Max = time.Unix(0, cnosql.MaxTime).UTC()
		}
	}
	return nil
}

func (c *compiledStatement) compile(stmt *cnosql.SelectStatement) error {
...
func (c *compiledStatement) compileFields(stmt *cnosql.SelectStatement) error {
	valuer := MathValuer{}
	...
	for _, f := range stmt.Fields {
		...
		if ref, ok := f.Expr.(*cnosql.VarRef); ok && ref.Val == "time" {
			if f.Alias != "" {
		...
		f.Expr = cnosql.Reduce(f.Expr, &valuer)
		field := &compiledField{
			global:        c,
			Field:         f,
			AllowWildcard: true,
...
func (c *compiledField) compileExpr(expr cnosql.Expr) error {
	...
	case *cnosql.BinaryExpr:
		...
		if _, ok := expr.LHS.(cnosql.Literal); ok {
			if _, ok := expr.RHS.(cnosql.Literal); ok {
				return errors.New("cannot perform a binary expression on two literals")
			}
			return c.compileExpr(expr.RHS)
		} else if _, ok := expr.RHS.(cnosql.Literal); ok {
...
// a nested expression.
func (c *compiledField) compileNestedExpr(expr cnosql.Expr) error {
...
func (c *compiledField) compileSymbol(name string, field cnosql.Expr) error {
...
func (c *compiledField) compileFunction(expr *cnosql.Call) error {
	...
	if expr.Name == "count" {
		...
		if arg0, ok := expr.Args[0].(*cnosql.Call); ok && arg0.Name == "distinct" {
			return c.compileDistinct(arg0.Args, true)
		} else if arg0, ok := expr.Args[0].(*cnosql.Distinct); ok {
			call := arg0.NewCall()
			return c.compileDistinct(call.Args, true)
		}
	}
...
func (c *compiledField) compilePercentile(args []cnosql.Expr) error {
...
func (c *compiledField) compileSample(args []cnosql.Expr) error {
	if exp, got := 2, len(args); got != exp {
...
func (c *compiledField) compileDerivative(args []cnosql.Expr, isNonNegative bool) error {
	name := "derivative"
	if isNonNegative {
	...
	if len(args) == 2 {
		switch arg1 := args[1].(type) {
		case *cnosql.DurationLiteral:
			if arg1.Val <= 0 {
				return fmt.Errorf("duration argument must be positive, got %s", cnosql.FormatDuration(arg1.Val))
			}
		default:
...
func (c *compiledField) compileElapsed(args []cnosql.Expr) error {
	if min, max, got := 1, 2, len(args); got > max || got < min {
...
func (c *compiledField) compileDifference(args []cnosql.Expr, isNonNegative bool) error {
	name := "difference"
	if isNonNegative {
...
func (c *compiledField) compileCumulativeSum(args []cnosql.Expr) error {
	if got := len(args); got != 1 {
...
func (c *compiledField) compileExponentialMovingAverage(name string, args []cnosql.Expr) error {
...
func (c *compiledField) compileKaufmans(name string, args []cnosql.Expr) error {
...
func (c *compiledField) compileChandeMomentumOscillator(args []cnosql.Expr) error {
	if got := len(args); got < 2 || got > 4 {
...
func (c *compiledField) compileIntegral(args []cnosql.Expr) error {
	if min, max, got := 1, 2, len(args); got > max || got < min {
...
func (c *compiledField) compileHoltWinters(args []cnosql.Expr, withFit bool) error {
	name := "holt_winters"
	if withFit {
	...
	n, ok := args[1].(*cnosql.IntegerLiteral)
	if !ok {
	...
	s, ok := args[2].(*cnosql.IntegerLiteral)
	if !ok {
...
func (c *compiledField) compileDistinct(args []cnosql.Expr, nested bool) error {
	if len(args) == 0 {
	...
	if _, ok := args[0].(*cnosql.VarRef); !ok {
...
func (c *compiledField) compileTopBottom(call *cnosql.Call) error {
	...
	limit, ok := call.Args[len(call.Args)-1].(*cnosql.IntegerLiteral)
	if !ok {
	...
	if _, ok := call.Args[0].(*cnosql.VarRef); !ok {
...
func (c *compiledField) compileMathFunction(expr *cnosql.Call) error {
...
func (c *compiledStatement) compileDimensions(stmt *cnosql.SelectStatement) error {
	for _, d := range stmt.Dimensions {
		// Reduce the expression before attempting anything. Do not evaluate the call.
		expr := cnosql.Reduce(d.Expr, nil)
		...
		case *cnosql.Call:
			...
			if expr.Name != "time" {
			...
			} else if lit, ok := expr.Args[0].(*cnosql.DurationLiteral); !ok {
				...
				if len(expr.Args) == 2 {
					...
					case *cnosql.Call:
						...
						expr.Args[1] = &cnosql.DurationLiteral{Val: c.Interval.Offset}
					case *cnosql.StringLiteral:
...
func (c *compiledStatement) validateCondition(expr cnosql.Expr) error {
	switch expr := expr.(type) {
	case *cnosql.BinaryExpr:
...
func (c *compiledStatement) subquery(stmt *cnosql.SelectStatement) error {
	...
	valuer := cnosql.MultiValuer(
		&cnosql.NowValuer{Now: c.Options.Now, Location: stmt.Location},
		&MathValuer{},
	)
	stmt.Condition = cnosql.Reduce(stmt.Condition, valuer)
	...
	if !subquery.Interval.IsZero() && subquery.FillOption == cnosql.NullFill {
		subquery.FillOption = cnosql.NoFill
	}
...
func (c *compiledStatement) Prepare(shardMapper ShardMapper, sopt SelectOptions) (PreparedStatement, error) {
	...
	if sopt.MaxBucketsN > 0 && !c.stmt.IsRawQuery && timeRange.MinTimeNano() == cnosql.MinTime {
		...
		if interval > 0 {
			...
			maxDiff := last - model.MinNanoTime
			if maxDiff/int64(interval) > int64(sopt.MaxBucketsN) {
				timeRange.Min = time.Unix(0, model.MinNanoTime)
			} else {
	...
	if !c.Interval.IsZero() && c.ExtraIntervals > 0 {
		if c.Ascending {
			...
			if !newTime.Before(time.Unix(0, cnosql.MinTime).UTC()) {
				timeRange.Min = newTime
			} else {
				timeRange.Min = time.Unix(0, cnosql.MinTime).UTC()
			}
		} else {
			newTime := timeRange.Max.Add(time.Duration(c.ExtraIntervals) * c.Interval.Duration)
			if !newTime.After(time.Unix(0, cnosql.MaxTime).UTC()) {
				timeRange.Max = newTime
			} else {
				timeRange.Max = time.Unix(0, cnosql.MaxTime).UTC()
			}
		}
	}
	...
	if sopt.MaxBucketsN > 0 && !stmt.IsRawQuery && c.TimeRange.MinTimeNano() > cnosql.MinTime {
